...
function DetailBody({ complaint }: { complaint: Complaint }) {
  ...
  const company = scoped.companies.find((c) => c.id === complaint.companyId)
  ...
  return (
    <>
      <DialogHeader className="border-b px-6 pt-5 pb-4 text-left">
        ...
        <DialogDescription>
          {company?.name} · 提交于 {formatDateTime(complaintCreatedAt(complaint))}
        </DialogDescription>
      </DialogHeader>
        ...
        <div className="space-y-5 px-6 py-5">
          <section>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border p-4 text-sm">
              <div>
                <p className="text-xs text-muted-foreground">投诉企业</p>
                <p className="mt-0.5">
                  {company ? (
              ...
              <div>
                <p className="text-xs text-muted-foreground">联系人 / 电话</p>
                <p className="mt-0.5">{company ? `${company.contactName} · ${company.contactPhone}` : '—'}</p>
              </div>
              <div>
...
/** 按状态渲染处理动作:派单 → 部门回复 → (企业不满意 → 主管回复)→ 关闭 */
  ...
  return (
    <section className="rounded-lg border border-primary/20 bg-primary/5 p-4">
      ...
      {status === 'supervisor' && (
        <ReplyForm
          label="主管介入回复"
          placeholder="主管复核后回复企业,给出最终解决方案"
          buttonText="提交主管回复"
          value={reply}
          onChange={setReply}
          onSubmit={() => {
      ...
      {status === 'replied' && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            已回复企业:企业可在企业端确认关闭,或对结果不满意申请主管介入。若已电话确认企业满意,可代为关闭。
          </p>
          <Button
            size="sm"
            variant="outline"
