...
function DetailBody({ workOrder }: { workOrder: WorkOrder }) {
  ...
  const household = state.households.find((h) => h.id === workOrder.householdId)
  ...
  return (
    <>
      <DialogHeader className="border-b px-6 pt-5 pb-4 text-left">
        ...
        <DialogDescription>
          {workOrderCategoryMap[workOrder.category]} · {household?.householdNo} · 报修于{' '}
        ...
        <div className="space-y-5 px-6 py-5">
          <section>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border p-4 text-sm">
              ...
              <InfoRow label="业主 / 电话">
                {household ? `${household.ownerName} · ${household.ownerPhone}` : '—'}
              </InfoRow>
              <InfoRow label="维修类型">{workOrderCategoryMap[workOrder.category]}</InfoRow>
              ...
              <InfoRow label="预约上门时间">{formatDateTime(workOrder.appointmentAt)}</InfoRow>
            </div>
          </section>
          ...
          {workOrder.satisfactionRating && (
            <section>
              <h4 className="mb-2 text-xs font-medium text-muted-foreground">业主满意度评价</h4>
...
/** 按当前状态渲染物业可执行的动作:接单 → 派单 → 预约 → 录入完工 */
function ActionZone({ workOrder }: { workOrder: WorkOrder }) {
  const state = useAppStore()
  const status = deriveWorkOrderStatus(workOrder)
  const { acceptWorkOrder, dispatchWorkOrder, setAppointment, submitCompletion } = state
  ...
  return (
    <section className="rounded-lg border border-primary/20 bg-primary/5 p-4">
      ...
      {status === 'dispatched' && (
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label className="text-xs">预约上门时间</Label>
            <Input
              type="datetime-local"
        ...
        <div className="space-y-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              toast.success('完工数据已录入,等待业主电子签字关单')
            }}
          >
            录入完工数据
          </Button>
      ...
      {status === 'done_pending_sign' && (
        <p className="text-sm text-muted-foreground">
          维修已完工,等待业主在业主端<span className="font-medium text-foreground">电子签字</span>确认关单。
        </p>
      )}
    </section>
  )
