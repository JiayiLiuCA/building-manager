...
/** mock 电子签字:确认即关单(SIGNED + CLOSED 事件,两端状态同步) */
export function SignatureDialog({ workOrder, open, onOpenChange }: Props) {
  ...
  const ownerName = useAppStore((s) => s.currentUser?.displayName ?? '业主')
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <PenLine className="size-4 text-primary" /> 电子签字确认
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="flex h-32 flex-col items-center justify-center rounded-lg border-2 border-dashed bg-muted/30">
          <p className="font-serif text-4xl italic tracking-wide text-foreground/80">{ownerName}</p>
          <p className="mt-2 text-xs text-muted-foreground">演示环境:点击下方按钮即代表手写签名确认</p>
        </div>
        <DialogFooter>
