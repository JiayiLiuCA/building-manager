...
export function ResidentComplaintDetailPage() {
  const { id = '' } = useParams()
  const state = useAppStore()
  const { closeComplaint, requestSupervisor } = state
  const complaint = state.complaints.find((c) => c.id === id)

  const [escalateOpen, setEscalateOpen] = useState(false)
  const [reason, setReason] = useState('')

  if (!complaint) return <Navigate to="/resident/complaints" replace />
        ...
        <CardContent className="p-4">
          ...
          {complaint.workOrderId && (
            <p className="mt-2 text-xs text-muted-foreground">
              关联工单:
              <Link to={`/resident/work-orders/${complaint.workOrderId}`} className="font-mono text-primary hover:underline">
                {complaint.workOrderId}
              </Link>
            </p>
          )}
      ...
      {status === 'replied' && (
        <Card className="border-violet-200 bg-violet-50/50 py-0">
          <CardContent className="space-y-3 p-4">
            <p className="text-sm font-medium">物业已回复,您对处理结果满意吗?</p>
      ...
      {status === 'supervisor' && (
        <Card className="border-red-200 bg-red-50/50 py-0">
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-xs text-red-600/80">您的诉求已升级,主管将尽快复核并回复您</p>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={escalateOpen} onOpenChange={setEscalateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>请说明您对当前处理结果不满意的原因,主管将复核并回复您</DialogDescription>
          </DialogHeader>
          <Textarea
            rows={3}
            placeholder="如:回复后问题依旧存在,未彻底解决"
            value={reason}
